...
describe('Cliente e2e test', () => {
  ...
  it('should load Clientes', async () => {
    ...
    expect(await clienteComponentsPage.getTitle()).to.eq('servicioventasjhApp.cliente.home.title');
  });
  ...
  it('should load create Cliente page', async () => {
    ...
    expect(await clienteUpdatePage.getPageTitle()).to.eq('servicioventasjhApp.cliente.home.createOrEditLabel');
  ...
  it('should create and save Clientes', async () => {
    ...
    await clienteComponentsPage.clickOnCreateButton();
    await promise.all([
      clienteUpdatePage.setCliente_idInput('5'),
      clienteUpdatePage.setNombreInput('nombre'),
      clienteUpdatePage.setApellidoInput('apellido')
    ]);
    expect(await clienteUpdatePage.getCliente_idInput()).to.eq('5', 'Expected cliente_id value to be equals to 5');
  ...
  it('should delete last Cliente', async () => {
    ...
    expect(await clienteDeleteDialog.getDialogTitle()).to.eq('servicioventasjhApp.cliente.delete.question');
