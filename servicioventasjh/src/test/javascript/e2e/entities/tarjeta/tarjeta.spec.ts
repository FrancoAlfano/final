...
describe('Tarjeta e2e test', () => {
  ...
  it('should load Tarjetas', async () => {
    ...
    expect(await tarjetaComponentsPage.getTitle()).to.eq('servicioventasjhApp.tarjeta.home.title');
  });
  ...
  it('should load create Tarjeta page', async () => {
    ...
    expect(await tarjetaUpdatePage.getPageTitle()).to.eq('servicioventasjhApp.tarjeta.home.createOrEditLabel');
  ...
  it('should create and save Tarjetas', async () => {
    ...
    await tarjetaComponentsPage.clickOnCreateButton();
    await promise.all([
      tarjetaUpdatePage.setTarjeta_idInput('5'),
      tarjetaUpdatePage.setCod_seguridadInput('5'),
      tarjetaUpdatePage.setVencimientoInput('2000-12-31'),
      tarjetaUpdatePage.setMontoInput('5'),
      tarjetaUpdatePage.setNumeroInput('5'),
      tarjetaUpdatePage.setTipoInput('tipo'),
      tarjetaUpdatePage.clienteSelectLastOption()
    ]);
    expect(await tarjetaUpdatePage.getTarjeta_idInput()).to.eq('5', 'Expected tarjeta_id value to be equals to 5');
    expect(await tarjetaUpdatePage.getCod_seguridadInput()).to.eq('5', 'Expected cod_seguridad value to be equals to 5');
    expect(await tarjetaUpdatePage.getVencimientoInput()).to.eq('2000-12-31', 'Expected vencimiento value to be equals to 2000-12-31');
    expect(await tarjetaUpdatePage.getMontoInput()).to.eq('5', 'Expected monto value to be equals to 5');
    expect(await tarjetaUpdatePage.getNumeroInput()).to.eq('5', 'Expected numero value to be equals to 5');
  ...
  it('should delete last Tarjeta', async () => {
    ...
    expect(await tarjetaDeleteDialog.getDialogTitle()).to.eq('servicioventasjhApp.tarjeta.delete.question');
